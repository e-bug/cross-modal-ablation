...
def parse_args():
    ...
    parser.add_argument("--threshold", default=0.4, type=float,
                        help="overlap threshold of image regions")
    parser.add_argument("--iou_type", default="standard", type=str,
                        help="overlap type of image regions")
    parser.add_argument("--no_mask_regions", default=False, action="store_true")
    # Optimizer
...
def main():
    ...
    train_dataset = ConceptCapLoaderTrain(args.annotations_path, args.features_path, tokenizer, args.bert_model,
                                          ...
                                          objective=args.objective, cache=cache, no_mask_regions=args.no_mask_regions,
                                          threshold=args.threshold, iou_type=args.iou_type,
    ...
    valid_dataset = ConceptCapLoaderVal(args.annotations_path, args.features_path, tokenizer, args.bert_model,
                                        ...
                                        threshold=args.threshold, iou_type=args.iou_type, 
                                        no_mask_regions=args.no_mask_regions, num_locs=config.num_locs)
    ...
    for epoch_id in range(start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataset):
            ...
            masked_loss_t, masked_loss_v, pair_match_loss = model(
                input_ids, image_feat, image_loc, 
                segment_ids, input_mask, image_mask, 
                lm_label_ids, image_label, image_cls, 
