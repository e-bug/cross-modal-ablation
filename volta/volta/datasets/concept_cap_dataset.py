...
def iot(anchors, gt_boxes):
    ...
    boxes = np.repeat(anchors.reshape(N, 1, 4), K, axis=1) # anchors.view(N, 1, 4).expand(N, K, 4)
    query_boxes = np.repeat(gt_boxes.reshape(1, K, 4), N, axis=0) # gt_boxes.view(1, K, 4).expand(N, K, 4)
...
class ConceptCapLoaderTrain(object):
    ...
    def __init__(
        ...
        preprocess_function = BertPreprocessBatch(
            ...
            threshold=threshold,
            iou_type=iou_type,
            no_mask_regions=no_mask_regions,
        )
...
class BertPreprocessBatch(object):
    def __init__(
        ...
        self.no_mask_regions = no_mask_regions
    ...
    def __call__(self, data):
        ...
        iou_func = iou
        if self.iou_type == "iot":
            iou_func = iot
        overlaps = iou_func(image_location_wp, image_location_wp)
    ...
    def random_region(self, image_feat, image_loc, num_boxes, overlaps):
        ...
        for i in range(num_boxes):
            ...
            if prob < 0.15 and not self.visualization:
                ...
                masked_label = np.logical_or(masked_label, overlaps[i] > self.threshold)  # FIXME
