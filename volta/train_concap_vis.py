...
from volta.datasets import ConceptCapVizLoaderTrain, ConceptCapVizLoaderVal
...
def main():
    ...
    # Datasets
    train_dataset = ConceptCapVizLoaderTrain(args.annotations_path, args.features_path, None, None, seq_len=1,
                                             batch_size=args.train_batch_size, num_workers=args.num_workers,
                                             local_rank=args.local_rank, objective=args.objective, cache=cache,
                                             add_global_imgfeat=config.add_global_imgfeat, num_locs=config.num_locs)
    valid_dataset = ConceptCapVizLoaderVal(args.annotations_path, args.features_path, None, None, seq_len=1,
                                           batch_size=args.train_batch_size, num_workers=2, objective=args.objective,
                                           add_global_imgfeat=config.add_global_imgfeat, num_locs=config.num_locs)
    ...
    for epoch_id in range(start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataset):
            ...
            _, masked_loss_v, _ = model(input_ids, image_feat, image_loc, segment_ids,
            ...
            if (step + 1) % args.grad_acc_steps == 0:
                ...
                if default_gpu:
                    tb_logger.step_train_CC(epoch_id, iter_id, float(0.0), float(masked_loss_v), float(0.0),
        ...
        for step, batch in enumerate(valid_dataset):
            ...
            if default_gpu:
                tb_logger.step_val_CC(epoch_id, float(0.0), float(masked_loss_v), float(0.0),
