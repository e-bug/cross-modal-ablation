...
from volta.train_utils import tbLogger, summary_parameters, save, resume
...
def main():
    ...
    train_dataset = ConceptCapLoaderTrain(args.annotations_path, args.features_path, tokenizer, args.bert_model,
                                          ...
                                          objective=args.objective, cache=cache, add_global_imgfeat="first", num_locs=5)
    valid_dataset = ConceptCapLoaderVal(args.annotations_path, args.features_path, tokenizer, args.bert_model,
                                        ...
                                        objective=args.objective, add_global_imgfeat="first", num_locs=5)
    ...
    if args.from_pretrained:
        ...
        raise NotImplementedError("train from scratch not implemented")
    ...
    for epoch_id in range(start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataset):
            ...
            lm_label_ids[lm_label_ids == -1] = -100
            output = model(input_ids, input_mask, segment_ids, labels=lm_label_ids)
            ...
            if (step + 1) % args.grad_acc_steps == 0:
                ...
                if default_gpu:
                    tb_logger.step_train_CC(epoch_id, iter_id, float(loss), 0.0, 0.0,
        ...
        for step, batch in enumerate(valid_dataset):
            ...
            if default_gpu:
                tb_logger.step_val_CC(epoch_id, float(loss), 0.0, 0.0, "TASK0", batch_size, "val")
