...
ix2cls_distr = pickle.load(open("flickr30k_entities_vg/ix2cls_distr.pkl", "rb"))
matching_ixs = []
for ix, cls_distr in ix2cls_distr.items():
    if np.argmax(ix2cls_distr[ix]) == vg_ix2label[ix]:
        matching_ixs.append(ix)
vg_ix2label = {ix: l for ix, l in vg_ix2label.items() if ix in matching_ixs}
vg_ixs = matching_ixs
...
df.to_csv("val_labelmatch-agree_%.1f_lang4vis.tsv" % thr, sep='\t')
